...
export async function generateDescription(
  base64FullImage,
  crop_box,
  detected_label, // Changed from global_caption to detected_label
  userId = "P1"
) {
  try {
    ...
    const response = await fetch(`${process.env.REACT_APP_API_BASE_URL}/describe`, {
      ...
      body: JSON.stringify({
        ...
        detected_label: detected_label, // YOLO detected object label (e.g., "person", "car", "dog")
        user_id: userId,
      }),
    });
    ...
    return {
      classification: data.classification, // 👈 Image classification category (e.g., "animal", "plant", "vehicle")
